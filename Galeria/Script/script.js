...
let autoRotateDelay = 800; // Cambia a let para poder modificarlo

function positionCards(animate = false, customDuration = null) {
    cards.forEach((card, i) => {
        ...
        if (animate) {
            gsap.to(card, {
                ...
                duration: customDuration !== null ? customDuration : 0.8,
                ease: 'power2.inOut'
            });
        } else {
            gsap.set(card, {
...
function updateCarousel(direction) {
    ...
    // Hacemos la transición más lenta al pasar las cards
    positionCards(true, 1.5); // 1.5 segundos de duración
}
...
autoRotateDelay = 3500; // Cambia la velocidad de auto rotación a más lenta
startAutoRotation();
